    ...
    ajaxDoneResult: function(result) {
        if (result.code == 1) {
            ...
            if (typeof result.msg == 'string') {
                $.formYlBookErrorMessages.push($.messageError(result.msg));
            } else {
                for (i in result.msg) {
                    $.formYlBookErrorMessages.push($.messageError(result.msg[i]));
                }
            }
        }
...
$.fn.extend({
     ...
     * @return {[type]} [description]
     */
    formYlBook: function(fields, is_ajax) {
        var me = $(this);
        ...
        $(this).submit(function(event) {
            for (var i = $.formYlBookErrorMessages.length - 1; i >= 0; i--) {
                $.formYlBookErrorMessages[i].hide();
            }
            $.formYlBookErrorMessages = [];
            ...
            if (typeof action == 'undefined' || action.length == 0) {
                $.formYlBookErrorMessages.push($.messageError('内部错误！'));
                // 启用按钮
                $.buttonEnable();
            } else {
                    ...
                    if (errors.length > 0) {
                        for (variable in errors) {
                            for (var i = 0; i < errors[variable].messages.length; i++) {
                                $.formYlBookErrorMessages.push($.messageError(errors[variable].messages[i]));
                            }
